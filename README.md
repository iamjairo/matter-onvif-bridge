...
The go2rtc container/image wiring lives under `docker/go2rtc/` (see the Dockerfile and `go2rtc.yaml`). Streams are registered dynamically by the bridge via the go2rtc REST API, so no manual stream configuration is needed.
...
│       └── go2rtc.yaml         # Read-only runtime config mounted by compose
└── scripts/
